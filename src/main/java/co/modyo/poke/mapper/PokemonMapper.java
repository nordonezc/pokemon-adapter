...
import co.modyo.poke.controller.EvolutionController;
import co.modyo.poke.controller.PokemonController;
...
public class PokemonMapper {
  ...
  public static Pokemon map(BasicInfo basicInfo, SpeciesInfo speciesInfo) {
    return Pokemon
      .builder()
      .description(
        DescriptionMapper.map(speciesInfo.getFlavorTextEntries(), "en")
      )
   ...
   */
  protected static LinkedID getLinkedIDFromURL(
    String givenURL,
    boolean isPokemonRelation
  ) {
  ...
  private static Integer getIDFromURL(String givenURL) {
    String urlWithoutLastBackSlash = givenURL.substring(
      0,
      givenURL.length() - 1
    );
   ...
   */
  private static LinkedID createLinkedID(
    Integer id,
    boolean isPokemonRelation
  ) {
    LinkedID linkedID = new LinkedID(id);
    Link evolutionLink = isPokemonRelation
      ? linkTo(methodOn(PokemonController.class).getPokemon(id))
        .withRel("pokemon")
      : linkTo(methodOn(EvolutionController.class).getEvolution(id))
        .withRel("evolution");
